...
func VoiceServersGET(store *mysql.DataStore) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		...
		voiceServers, err := store.GetVoiceServers()
		if err != nil {
		...
		return render(ctx, http.StatusOK, "admin/voice_servers", map[string]interface{}{
			"game":           "",
...
func VoiceServersPOST(store *mysql.DataStore) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		...
		return ctx.Redirect(http.StatusFound, "/admin/voicecomm")
	}
}
