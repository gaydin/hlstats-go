                                ...
                                {{ range $i, $game := .games }}
                                    <tr>
                                        <th scope="row"><img src="/public/hlstatsimg/game.gif" width="16" height="16"
                                                             border="0"
                                                             alt=""/></th>
                                        <td><input type="hidden" name="rows[]" value="{{ $game.Code }}" />{{ $game.Code }}</td>
                                        <td><input type="text" name="{{ $game.Code }}_name" class="form-control" value="{{ $game.Name }}"></td>
                                        <td><select name="{{ $game.Code }}_realgame">
                                                {{ range $root.supported_games }}
                                                    <option value="{{ .Code }}" {{ if eq $game.RealGame .Code }}selected="selected"{{ end }}>{{ .Name }}</option>
                                                {{ end }}
                                            </select></td>
                                        <td>
